...
import type { SamplesSummary } from "../hooks/useSamplesRows";
  ...
  return (
    ...
    >
      <CardContent sx={{ py: 1.5, "&:last-child": { pb: 1.5 } }}>
        ...
        <Typography variant="h6" aria-hidden="true" sx={{ fontWeight: 600 }}>
          {value}
        </Typography>
      </CardContent>
    </Card>
...
export default function SamplesSummaryBar({ summary }: SamplesSummaryProps) {
  return (
    ...
    >
      ...
      <SummaryCard
        label="Avg Adjusted Moisture"
        value={`${summary.avgMoisture.toFixed(4)} %`}
      />
      <SummaryCard
        label="Avg Adjusted Density"
        value={`${summary.avgDensity.toFixed(4)} g/cm³`}
      />
    </Box>
  );
}
