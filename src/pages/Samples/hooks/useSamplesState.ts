...
export function useSamplesState() {
  ...
  const visibleRows = useMemo(() => {
    ...
    return rows.filter((r) => r.sampleId.toLowerCase().includes(lower))
  }, [rows, filter])

  const summary: SamplesSummary = useMemo(() => {
    if (visibleRows.length === 0) return { avgMoisture: 0, avgDensity: 0, totalSamples: 0 }
    const avgMoisture =
      visibleRows.reduce((sum, r) => sum + r.adjustedMoisture, 0) / visibleRows.length
    const avgDensity =
      visibleRows.reduce((sum, r) => sum + r.adjustedDensity, 0) / visibleRows.length
